...
#define BAUDRATE              1000000           // Default Baudrate of DYNAMIXEL X series (57600)
...
{  
  x_command = msg.pose.position.x*10000;
  ...
  // When writing 2 byte data to AX / MX(1.0), use write2ByteTxRx() instead.
  dxl_comm_result = packetHandler->write4ByteTxRx(
    portHandler, (uint8_t)Id, ADDR_GOAL_POSITION, x_command, &dxl_error);
  if (dxl_comm_result == COMM_SUCCESS) {
    ROS_INFO("setPosition : [ID:%d] [POSITION:%lf]", Id, x_command);
  } else {
    ROS_ERROR("Failed to set position! Result: %d", dxl_comm_result);
  }
}
...
void setPositionCallback(const dynamixel_sdk_examples::SetPosition::ConstPtr & msg)
{
  ...
  dxl_comm_result = packetHandler->write4ByteTxRx(
    portHandler, (uint8_t)Id, ADDR_GOAL_POSITION, x_command, &dxl_error);
  if (dxl_comm_result == COMM_SUCCESS) {
    ROS_INFO("setPosition : [ID:%d] [POSITION:%d]", Id, x_command);
  } else {
    ROS_ERROR("Failed to set position! Result: %d", dxl_comm_result);
  }
}
...
int main(int argc, char ** argv)
{
  ...
  ros::Publisher dynPub = nh.advertise<geometry_msgs::Twist>("Now_endEffector_Position", 10);
  ...
  // When reading 2 byte data from AX / MX(1.0), use read2ByteTxRx() instead.
  dxl_comm_result = packetHandler->read4ByteTxRx(
    portHandler, 1, ADDR_PRESENT_POSITION, (uint32_t *)&position, &dxl_error);
  publisher.linear.x = position;
    dynPub.publish(publisher);
  ros::spinOnce();
  loop.sleep();
  }
  portHandler->closePort();
