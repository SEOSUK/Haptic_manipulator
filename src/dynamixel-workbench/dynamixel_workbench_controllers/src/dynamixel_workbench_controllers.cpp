    ...
    for (auto const& info:dynamixel_info_)
    {
      if (dxl.first == info.first)
      {
        if (info.second.item_name != "ID" && info.second.item_name != "Baud_Rate")
        {
          ...
          //bool resultt = dxl_wb_->itemWrite(12,"Position_P_Gain",200,&log);
          if (result == false)
          {
            ROS_ERROR("%s", log);
          ...
          /* if (resultt == false)
          {
            ROS_ERROR("Failed to write value[%d] on items[%s] to Dynamixel[ID : %d]", 200, "Position_P_Gain",12);
            return false;
          }
          else{ROS_INFO("success to itemwrite!");} */
        }
      }
    }
